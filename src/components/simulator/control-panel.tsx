...
import { RotateCcw, Save, Sliders, Upload } from "lucide-react";
...
export function ControlPanel({ side, config }: { side: "left" | "right"; config: EnvironmentConfig }) {
  ...
  const totalControls = groups.reduce((sum, group) => sum + group.controls.length, 0);

  return (
    <aside className={`panel-shell ${side === "right" ? "panel-shell-right" : ""} flex h-full min-h-0 flex-col`}>
      {/* Panel header */}
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-[var(--line)] px-3">
        <div className="flex items-center gap-2.5">
          <span className="hud-corner relative flex h-7 w-7 items-center justify-center border border-[var(--line-strong)] bg-[var(--bg-deepest)] text-[var(--accent-cyan)]">
            <Sliders size={12} />
          </span>
          <div className="leading-tight">
            <p className="mono text-[9px] font-bold uppercase tracking-[0.22em] text-[var(--text-faint)]">
              {side === "left" ? "MOD.A · DEMAND" : "MOD.B · OPERATIONS"}
            </p>
            <h2 className="mt-0.5 text-[12.5px] font-semibold tracking-wide text-[var(--text-primary)]">
              {side === "left" ? "외부 / 고객 조건" : "운영 / 공간 조건"}
            </h2>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="mono text-[9.5px] uppercase tracking-[0.16em] text-[var(--text-faint)]">
            {totalControls.toString().padStart(2, "0")} VAR
          </span>
          {side === "right" ? (
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        <div className="grid gap-3">
          {groups.map((group, groupIndex) => (
            <section key={group.id} className="section-block">
              {/* Group header */}
              <div className="mb-3 flex items-center justify-between gap-2 border-b border-[var(--line)] pb-2">
                <div className="flex items-center gap-2">
                  <span className="mono text-[9px] font-bold tracking-[0.18em] text-[var(--text-faint)]">
                    {String(groupIndex + 1).padStart(2, "0")}
                  </span>
                  <span className="h-3 w-px bg-[var(--line-strong)]" />
                  <h3 className="text-[11.5px] font-semibold uppercase tracking-[0.06em] text-[var(--text-primary)]">
                    {group.title}
                  </h3>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="mono text-[9px] uppercase tracking-[0.16em] text-[var(--text-muted)]">
                    {group.code}
                  </span>
                  <span className="h-1.5 w-1.5 bg-[var(--accent-cyan)] beacon" />
                </div>
              </div>
          ...
          <div className="border border-[var(--line)] bg-[var(--bg-raised)] px-3 py-2 mono text-[9.5px] uppercase tracking-[0.16em] text-[var(--text-faint)]">
            <span className="text-[var(--text-secondary)]">› </span>
            모든 변수 변경은 즉시 시뮬레이션에 반영됩니다.
          </div>
        </div>
      </div>
    </aside>
